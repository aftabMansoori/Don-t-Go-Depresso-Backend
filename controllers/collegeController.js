...
module.exports.getEmailExcel = catchAsync(async(req,res,next)=>{
  ...
  let collegeEmailsList = await Promise.all(collegeData.studentMails.map(el=>{
    return el.studentMail;
  }))
  let xlsFormat = await Promise.all(collegeEmailsList.map(async email=>{
    let row = {};
    row.email = email;
    return row;
})
...
module.exports.addEmailExcel = catchAsync(async(req,res,next)=>{
    ...
    await session.withTransaction(async () => {
        ...
        await College.findOneAndUpdate({ collegeCode: req.user.collegeCode },{$push : {studentMails : {$each : mail}}});
        return createdMails;
    });
    res.status(201).json({
